...
func gatewayCmd(debug bool) error {
	...
	if cfg.EmailWebhook.Enabled {
		registerEmailWebhook(healthServer, cfg, cfg.WorkspacePath())
		fmt.Println("✓ Email webhook enabled at /webhook/email")
	}
	go func() {
...
func registerEmailWebhook(healthServer *health.Server, cfg *config.Config, workspace string) {
	emailDir := filepath.Join(workspace, "emails")

	healthServer.RegisterHandler("/webhook/email", func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			http.Error(w, "Method Not Allowed", http.StatusMethodNotAllowed)
			return
		}

		if cfg.EmailWebhook.Secret != "" && r.Header.Get("X-Pico-Auth") != cfg.EmailWebhook.Secret {
		...
		filePath := filepath.Join(emailDir, date+".jsonl")
